	...
	implements \Aimeos\MShop\Common\Factory\Iface
{
	 ...
	 * @throws \Aimeos\MShop\Sardes\Exception|\Aimeos\MShop\Exception If requested manager
	...
	{
		if( $name === null ) {
			$name = $context->getConfig()->get( 'sardes/manager/name', 'Standard' );
		}

		$iface = \Aimeos\MShop\Sardes\Manager\Iface::class;
		$classname = '\Aimeos\MShop\Sardes\Manager\\' . $name;
		...
		return self::addManagerDecorators( $context, $manager, 'sardes' );
	}
