...
<?php /* <div class="locale-select-language">
	...
	<!-- <ul class="ht-dropdown locale-select-language"> -->
		<li class="desktop-menu-item select-dropdown select-current"><a href="#" title="<?= $this->translate( 'client', 'Select language' ); ?>"><img class="locale-icon" src="<?= sardes_url('/assets/img/lang/').''.$this->get( 'selectLanguageId', 'en' )?>.png?v=1"><i class="ion-chevron-down"></i><i class="icon ion-ios-plus-empty" ></i></a>
			<ul class="ht-dropdown locale-select-language">
				<?php foreach( $this->get( 'selectMap', [] ) as $lang => $list ) : ?>
					<li class="select-item <?= ( $lang === $this->get( 'selectLanguageId', 'en' ) ? 'active' : '' ); ?>">
						<a href="<?= $enc->attr( $this->url( $this->request()->getTarget(), $this->param( 'controller' ), $this->param( 'action' ), array_merge( $this->get( 'selectParams', [] ), $list[$this->get( 'selectCurrencyId', 'EUR' )] ?? current( $list ) ), [], $config ) ); ?>">
							<img  class="locale-icon" src="<?= sardes_url('/assets/img/lang/').''.$lang?>.png" > <?= $enc->html( $this->translate( 'language', $lang ), $enc::TRUST ); ?>
						</a>
