...
<section class="aimeos catalog-stage <?= $enc->attr( $this->get( 'stageCatPath', map() )->getConfigValue( 'css-class', '' )->join( ' ' ) ); ?>" data-jsonurl="<?= $enc->attr( $this->url( $optTarget, $optCntl, $optAction, [], [], $optConfig ) ); ?>">
	...
	<?php /*?><?php if( !( $mediaItems = $this->get( 'stageMediaItems', map() ) )->isEmpty() ) : ?>
			...
			<ol>
				...
				<?php if( isset( $this->stageCatPath ) ) : ?>
					<?php foreach( $this->get( 'stageCatPath', map() ) as $cat ) : ?>
						<li>
							<?php $home=$cat->getName( 'url' );
							if ($home=='Home') :?>
							<a href="<?= route('home',['locale'=>Route::current()->parameter('locale','en'),'currency'=>Route::current()->parameter('currency','EUR')])?>">
									<?= $enc->html( $cat->getName() ); ?>
							<?php else : ?>
							<a href="<?= route('aimeos_shop_list',array_merge( ['locale'=> \Route::current()->parameter('locale','ru'), 'currency'=> \Route::current()->parameter('currency','RUB')], ['f_name' => $cat->getName( 'url' ), 'f_catid' => $cat ->getId()] ))?>">
								<?= $enc->html( $cat->getName() ); ?>
							</a>
							<?php endif; ?>
						</li>
