...
<?php foreach( $prices as $priceItem ) : ?>
	...
	<div class="price-item <?= $enc->attr( $priceItem->getType() ); ?>" itemprop="priceSpecification" itemscope="" itemtype="http://schema.org/PriceSpecification">
		...
		<del><?= $enc->html( sprintf( $format['value'], $this->number( ($priceItem->getValue() + $priceItem->getRebate()), $priceItem->getPrecision() ), $currency ), $enc::TRUST ); ?></del>
		</span>
        <span class="rebate product__tag m-l-15 btn--tiny btn--green">
