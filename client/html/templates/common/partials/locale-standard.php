...
<?php if(sardes_option('show_language_select')){?>
	<li class="select-dropdown user-currency pos-relative"> <a class="user-set-role__button" href="#" data-toggle="dropdown" aria-expanded="false"><img src="<?=sardes_url('assets/img/icon/flag/icon_'.$this->get( 'selectLanguageId', 'en' ).'.png')?>" alt="">•
    ...
    <ul class="expand-dropdown-menu dropdown-menu locale-select-language" >
      ...
      <li class="<?= ( $lang === $this->get( 'selectLanguageId', 'en' ) ? 'active' : '' ); ?>"> <a href="<?= $enc->attr( $this->url( $this->request()->getTarget(), $this->param( 'controller' ), $this->param( 'action' ), array_merge( $this->get( 'selectParams', [] ), $list[$this->get( 'selectCurrencyId', 'EUR' )] ?? current( $list ), $url_parameters ), [], $config ) ); ?>"> <img src="<?=sardes_url('assets/img/icon/flag/icon_'.$lang.'.png')?>" alt="">
    ...
    <?php	} ?>
    <?php if(sardes_option('show_currency_select')){?>  
